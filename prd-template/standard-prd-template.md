---
type: prd-template
feature: standard-prd-template
version: "1.5"
status: active
source_doc: prd-template/README.md；prd-template/prd-writing-workflow.md；knowledge-base/_kb-ingestion-process.md；用户确认结论 2026-05-05
source_section: "standard PRD template；PRD fact rules；功能粒度与拆分规则；research companion file rules"
last_updated: 2026-05-05
owner: 吴忆锋
readers: [product, ui, dev, qa, business, ai]
---

# Standard PRD Template 标准 PRD 模板

## 0. 功能粒度判断

> 写 PRD 前，必须先判断当前文档承接的是一个独立功能，还是多个独立功能。  
> 标准 PRD 默认只描述一个独立功能。  
> 如果同一原始 PRD 或同一模块中包含多个独立功能，应拆成多个 PRD 文件，并用模块 `_index.md` 做索引和边界说明。

### 0.1 独立功能判断标准

满足以下任一条件，通常应拆成独立 PRD：

| 判断维度 | 拆分信号 | 示例 |
|---|---|---|
| 目标不同 | 解决的用户 / 业务问题不同 | 卡资金回退 vs 卡交易记录查询 |
| 触发不同 | 由不同事件或入口触发 | DTC 通知触发 vs 用户点击交易历史 |
| 用户不同 | 一个是后台自动流程，一个是用户页面流程 | 自动归集 vs 用户查看详情 |
| 页面不同 | 页面、入口、交互路径不同 | Card Home 最近交易 vs Transaction History |
| 业务结果不同 | 成功结果和失败结果不同 | 资金入 Wallet vs 展示交易列表 |
| 责任方不同 | 主责任系统或外部依赖不同 | Card 服务处理资金回退 vs Transaction 服务查询记录 |
| 状态模型不同 | 状态枚举、流转或异常处理不同 | 回退成功 / 失败 vs 交易展示状态 |
| 验收不同 | 测试场景无法用同一组验收标准覆盖 | Webhook 处理验收 vs 列表筛选验收 |

### 0.2 不应混写的典型情况

以下内容不得强行写进同一个功能 PRD：

| 情况 | 正确处理 |
|---|---|
| 一个是业务处理流程，一个是记录查询流程 | 拆成两个 PRD，分别写处理和查询 |
| 一个是后台自动流程，一个是用户页面流程 | 后台流程写业务处理 PRD，页面流程写页面 / 查询 PRD |
| 一个是主流程，一个只是后续展示入口 | 主流程 PRD 只写入口边界，展示功能另写 PRD |
| 一个原始 PRD 包含多个能力 | 按能力拆文件，原始 PRD 名称只作为共同来源引用 |
| 只是共用字段、接口或状态 | 共用内容放到字段 / 状态 / 索引或公共规则，不因此合并 PRD |

### 0.3 允许放在同一 PRD 的内容

以下内容可以放在同一 PRD：

1. 同一个功能的主流程、分支流程、异常流程。
2. 同一个功能下的页面跳转和页面状态。
3. 同一个功能必须依赖的字段、接口、通知、权限、合规和验收。
4. 同一个功能内的成功、失败、重试、终态等状态规则。

### 0.4 多功能模块的写法

如果一个模块包含多个功能，应采用以下结构：

```text
module/
├── _index.md              # 模块索引，只写功能边界和路由
├── feature-a.md           # 独立功能 A PRD
├── feature-b.md           # 独立功能 B PRD
└── feature-c.md           # 独立功能 C PRD
```

`_index.md` 只回答：

1. 模块有哪些功能。
2. 每个功能对应哪个 PRD 文件。
3. 原始 PRD 与知识库文件的映射关系。
4. 功能之间的边界和引用关系。
5. 不应该把哪些功能混写。

---

## 1. 文档信息

| 项目 | 内容 |
|---|---|
| 功能名称 | TBD |
| 所属模块 | TBD |
| Owner | TBD |
| 版本 | TBD |
| 状态 | Draft / Review / Approved |
| 更新时间 | TBD |
| 来源文档 | TBD |
| Brief | TBD |
| Research | 不适用 / `_research-<feature>.md` |

---

## 2. 需求背景、目标与范围

### 2.1 需求背景

说明为什么要做这个需求。只写已确认背景，不写推测。

### 2.2 用户问题 / 业务问题

说明当前用户、业务、合规、运营或系统上存在什么问题。

### 2.3 需求目标

说明本次需求要达成什么结果。

### 2.4 涉及功能清单

> 这里只列本 PRD 直接承接的功能点。  
> 如果发现功能点属于另一个独立功能，应标记为 Out of Scope，并在说明中写明对应 PRD 或待拆分位置。  
> 不要因为多个功能来自同一原始文档，就把它们写进同一个 PRD。

| 功能点 | 本期范围 | 优先级 | 状态 | 说明 |
|---|---|---|---|---|
| TBD | In Scope / Deferred / Out of Scope | P0 / P1 / P2 | Confirmed / Open | TBD |

### 2.5 功能边界与拆分结论

> 本节用于明确当前 PRD 为什么是一个独立功能，以及哪些相关能力不在本文中展开。  
> 如果本功能与其他功能强相关，只写边界和引用，不展开其他功能的完整流程。

| 相关能力 | 与本文关系 | 当前处理 | 对应文件 / 位置 |
|---|---|---|---|
| TBD | 主功能 / 前置能力 / 后续展示 / 查询能力 / 公共规则 | 本文展开 / 只引用 / Out of Scope / 待拆分 | TBD |

---

## 3. 业务流程与规则

> 本章节说明功能如何运转。  
> 业务规则写系统如何判断和流转；页面规则写用户看到什么、如何操作、如何反馈。  
> 同一规则不要在业务章节和页面章节重复写，页面章节只承接业务规则的用户表现。  
> 本章节只写当前独立功能的业务流程，不展开其他独立功能。  
> 如果流程中出现另一个独立功能，只写“进入 / 调用 / 引用 XXX 功能”，不要继续展开其完整流程。

### 3.1 业务主流程说明

用自然语言简要说明主流程。

主流程说明必须遵守：

1. 只描述当前功能的业务流转。
2. 不把后续查询、历史记录、详情页、报表、通知中心等独立能力混入主流程。
3. 不把接口字段、幂等键、Header、返回码、参数名写成主流程。
4. 技术实现细节放到第 5 章，未确认技术方案放到第 8 章。

### 3.2 业务时序图

> 本图是业务时序图，用 `sequenceDiagram` 表达泳道式业务流转。  
> 重点说明用户、产品前端、业务服务、外部能力之间的业务责任、关键判断和阶段流转。  
> 不承载具体交互细节、页面元素规则、错误文案或接口设计；这些内容以交互稿、页面章节、异常表和相关模块文档为准。  
> 参与方命名应使用业务语义，例如“用户 / AIX App / 账户服务 / 身份验证服务”，避免写成纯技术角色或接口名。  
> 图中只保留关键业务节点，不展开输入框实时校验、按钮禁用、Toast、Popup 具体文案、接口字段、接口返回码、幂等字段、Header、请求参数等细节。  
> 图中不得展开其他独立功能的完整流程；如需跳转或引用，用“进入 XXX 功能 / 由 XXX 模块承接”表达。

```mermaid
sequenceDiagram
    autonumber
    actor 用户
    participant App as AIX App
    participant Service as 业务服务
    participant External as 外部能力 / 外部系统

    Note over 用户,External: A. 当前功能业务阶段名称
    用户->>App: 发起当前功能的业务动作
    App->>Service: 发起业务校验或业务处理
    Service->>Service: 执行业务判断

    alt 业务判断不通过
        Service-->>App: 返回不通过原因
        App-->>用户: 停留当前阶段，按页面规则提示
    else 业务判断通过
        Service->>External: 发起外部能力或下一阶段处理
        External-->>Service: 返回业务处理结果
        Service-->>App: 返回当前功能结果
        App-->>用户: 展示当前功能结果或进入下一功能入口
    end
```

### 3.3 流程步骤与业务规则

> 主流程、分支、前置条件、责任方、成功 / 失败结果统一放在这里。  
> 不要再额外拆成“前置条件”“分支规则”“主规则”多个重复章节。  
> 如果某一步只是调用另一个独立功能，只写调用边界，不展开另一个功能的步骤。

| 步骤 | 场景 / 规则 | 触发条件 | 责任方 | 系统处理 | 成功结果 | 失败 / 分支结果 | 来源 |
|---|---|---|---|---|---|---|---|
| 1 | TBD | TBD | User / App / Backend / External | TBD | TBD | TBD | TBD |

### 3.4 状态规则

> 只放有明确状态枚举或状态流转的内容。  
> 如果没有状态机，本节可写“不适用”。  
> 不要把其他独立功能的状态模型并入本文。

| 状态 | 含义 | 触发条件 | 用户可见表现 | 系统处理 | 可迁移到 | 是否终态 | 来源 |
|---|---|---|---|---|---|---|---|
| TBD | TBD | TBD | TBD | TBD | TBD | 是 / 否 | TBD |

### 3.5 业务级异常与失败处理

> 只放跨页面、跨系统、业务级异常。  
> 页面内输入错误、Toast、按钮禁用、权限提示放到对应 Page 下。  
> 不要把其他独立功能的失败场景并入本文；只写与本文直接相关的异常。

| 异常场景 | 触发条件 | 错误来源 | 错误码 / 原因 | 用户表现 | 系统处理 | 是否可重试 | 最终状态 |
|---|---|---|---|---|---|---|---|
| TBD | TBD | App / Backend / External | TBD | TBD | TBD | 是 / 否 | TBD |

---

## 4. 页面与交互说明

> 本章节说明用户看到什么、怎么操作、页面如何流转。  
> 页面流程图只表达当前功能内的页面跳转。  
> 每个 Page 下面聚合：截图、页面目标、展示内容、用户动作、系统处理、元素规则、状态、提示、异常分支。  
> 不单独做页面截图索引。  
> 如果某个页面属于另一个独立功能，只写入口或跳转边界，不展开页面细节。

### 4.1 页面关系总览图

> 本图只表达页面之间的主关系，用于快速理解页面跳转。  
> 主节点必须是 Page；用户动作写在线条上。  
> 输入校验、账户拦截、认证失败、Toast、弹窗、权限提示、后端动作、通知、外部账户模型等细节不放入总览图，应放到对应 Page 的页面说明、元素明细或异常分支中。  
> 不要在一个页面关系图中混入另一个独立功能的完整页面链路。

```mermaid
flowchart LR
  PageA[Page A]
  PageB[Page B]
  PageC[Page C]
  PageD[Page D]

  PageA -->|User action| PageB
  PageB -->|Success / next action| PageC
  PageB -->|Alternative action| PageD
```

### 4.2 Page Name

![Page Name](./assets/<feature>/<page-name>.png)

| 区块 | 内容 |
|---|---|
| 页面类型 | 主页面 / 状态页 / 错误页 / 成功页 / 外部 H5 |
| 页面目标 | TBD |
| 入口 / 触发 | TBD |
| 展示内容 | TBD |
| 用户动作 | TBD |
| 系统处理 / 责任方 | TBD |
| 元素 / 状态 / 提示规则 | TBD |
| 成功流转 | TBD |
| 失败 / 异常流转 | TBD |
| 备注 / 边界 | TBD |

#### 元素明细（复杂页面才需要）

> 简单页面不要强行补元素明细。  
> 只有页面元素多、状态多、校验多、提示多时才补。

| 元素 / 状态 / 提示 | 类型 | 触发 / 展示条件 | 交互 / 校验规则 | 成功结果 | 失败 / 提示 | 后续流转 | 文案来源 |
|---|---|---|---|---|---|---|---|
| TBD | Button / Input / Selector / Link / Text / Toast / Popup / Permission Prompt / State | TBD | TBD | TBD | TBD | TBD | TBD |

---

## 5. 字段、接口与数据

> 只写实现、联调、测试必须知道的字段、接口和数据。没有则写“不适用”。  
> 字段、接口、日志、幂等键、Header、返回码、参数名等技术细节放在本章，不放在业务主流程说明或业务时序图中。  
> 如果字段或接口属于另一个独立功能，只引用对应 PRD，不在本文展开完整字段表。

| 类型 | 名称 | 所属系统 | 来源 | 用途 | 规则 / 输入输出 | 异常处理 |
|---|---|---|---|---|---|---|
| 字段 / 接口 / 数据 / 埋点 / 日志 | TBD | AIX / DTC / AAI / KUN / WalletConnect / OS | 用户输入 / 后端返回 / 外部系统 | TBD | TBD | TBD |

---

## 6. 通知规则（如适用）

> 没有通知则写“不适用”。  
> 只写当前功能直接触发的通知。其他功能触发的通知只做引用，不展开模板。

| 触发事件 | 通知渠道 | 通知对象 | 文案 / 模板 | 跳转目标 | 失败 / 补发规则 |
|---|---|---|---|---|---|
| TBD | Email / Push / In-app | TBD | TBD | TBD | TBD |

---

## 7. 权限 / 合规 / 风控（如适用）

> 只写与本需求直接相关的权限、合规、风控要求。没有则写“不适用”。  
> 不要把模块级、系统级或其他独立功能的合规规则全部搬进本文。

| 类型 | 规则 | 影响 | 来源 |
|---|---|---|---|
| 用户权限 / 系统权限 / 合规 / 风控 / 隐私 | TBD | TBD | TBD |

---

## 8. 待确认事项

> 只写影响当前功能验收或边界判断的待确认项。  
> 如果待确认项本质上是在判断是否要拆分功能，应优先回到第 0 章判断功能粒度。

| 问题 | 影响范围 | 当前处理 | 是否阻塞验收 | 建议确认人 |
|---|---|---|---|---|
| TBD | TBD | 不阻塞 / 阻塞 / Deferred | 是 / 否 | TBD |

---

## 9. 验收标准 / 测试场景

### 9.1 验收标准

> 验收标准必须覆盖当前独立功能，不应把另一个独立功能的验收标准混入本文。  
> 如果一个验收标准需要完全不同的前置条件、触发方式和系统结果，通常说明它应拆到另一个 PRD。

| 验收场景 | 验收标准 |
|---|---|
| 正常流程 | TBD |
| 异常流程 | TBD |
| 页面展示 | TBD |
| 系统交互 | TBD |
| 通知 | TBD |
| 数据 / 埋点 | TBD |

### 9.2 测试场景矩阵

| 场景 | 前置条件 | 用户操作 | 预期页面表现 | 预期系统结果 | 是否必测 |
|---|---|---|---|---|---|
| TBD | TBD | TBD | TBD | TBD | 是 / 否 |

---

## 10. 来源引用

> 如果一个原始文档拆成多个 PRD，来源引用必须标明本文使用的原文章节。  
> 不得因为引用了同一原始文档，就把该原始文档中的所有功能都写进本文。  
> 如存在 `_research-<feature>.md`，本节可以引用其路径作为调研过程材料，但 research 不能单独作为确认事实来源。PRD 中被采纳为事实的内容必须引用原始来源文件或用户确认结论。

- Brief: `_brief-<feature>.md`
- Research: 不适用 / `_research-<feature>.md`
- (Ref: TBD)
