...
stateDiagram-v2
    ...
    UPDATE_FAILED --> NEW_EMAIL_INPUTTING: 返回重新输入或重新发送验证码
    SUCCESS --> [*]
```
...
| UPDATE_FAILED | Account 更新失败或新邮箱被并发占用 | 返回重新输入、返回个人中心 | 用户选择重试路径或退出 | 不自动重试；如需继续必须返回 New Email Input，必要时重新发送并验证新邮箱 OTP | Account 邮箱保持原邮箱 |
...
**页面目的**  
告知用户邮箱更换未完成，账户邮箱仍保持原邮箱，并提供重新尝试或返回路径。
...
| 场景 / 处理项 | 规则 | 结果 |
|---|---|---|
| 点击 Try again | 返回 New Email Input | 用户需重新确认新邮箱；如继续使用同一新邮箱，需重新发送并验证新邮箱 OTP |
| 点击返回个人中心 | 返回个人中心 | 个人中心 Security 区域 Email 行仍展示原邮箱 |
