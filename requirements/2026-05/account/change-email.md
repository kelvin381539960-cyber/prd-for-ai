...
sequenceDiagram
    ...
    alt 旧邮箱 OTP 发送失败
        App-->>User: 按 Email OTP 发送失败规则处理，不进入旧邮箱 OTP 验证页
    else 旧邮箱 OTP 发送成功
        ...
        alt 旧邮箱 OTP 未通过
            ...
            alt 新邮箱不可用
                ...
                alt 新邮箱 OTP 发送失败
                    App-->>User: 停留新邮箱输入页，按 Email OTP 发送失败规则处理
                else 新邮箱 OTP 发送成功
                    ...
                    alt 新邮箱 OTP 未通过
                        ...
                        Account-->>App: 返回邮箱更新结果

                        alt 更新成功
                            Account->>Notification: 触发旧邮箱 / 新邮箱 Email 通知
                            App-->>User: 展示更换成功
                        else 更新失败 / 新邮箱已被并发占用
                            App-->>User: 展示 Change Email Failed，账户邮箱保持原邮箱
                        end
                    end
                end
            end
...
| 旧邮箱 OTP 发送失败 | 复用 Email OTP Verification 发送失败规则 | 不进入 Old Email OTP Verify Page；账户邮箱不得更新 | `knowledge-base/security/email-otp-verification.md` |
| 新邮箱可用性校验通过 | 请求发送新邮箱 OTP，不更新账户邮箱 | 新邮箱 OTP 发送成功后进入 New Email OTP Verify Page | 本 PRD |
| 新邮箱 OTP 发送失败 | 复用 Email OTP Verification 发送失败规则 | 停留 New Email Input；账户邮箱不得更新 | `knowledge-base/security/email-otp-verification.md` |
...
| 邮箱更新时发现新邮箱已不可用 | 更新前再次校验邮箱全局唯一性，避免并发占用 | 进入 Change Email Failed，账户邮箱保持原邮箱，提示 `This email has been used` | 本 PRD |
| 账户邮箱更新失败 | 保持原邮箱，不产生半更新状态 | 进入 Change Email Failed，提示 `Something went wrong. Please try again later` | 本 PRD |
...
stateDiagram-v2
    [*] --> INIT
    INIT --> OLD_EMAIL_OTP_SENDING: 点击开始更换
    OLD_EMAIL_OTP_SENDING --> OLD_EMAIL_OTP_SEND_FAILED: 旧邮箱 OTP 发送失败
    OLD_EMAIL_OTP_SENDING --> OLD_EMAIL_OTP_SENT: 旧邮箱 OTP 发送成功
    OLD_EMAIL_OTP_SENT --> OLD_EMAIL_OTP_FAILED: 旧邮箱 OTP 失败 / 锁定 / 过期 / 重发超限
    OLD_EMAIL_OTP_SENT --> OLD_EMAIL_VERIFIED: 旧邮箱 OTP 验证成功
    OLD_EMAIL_VERIFIED --> NEW_EMAIL_INPUTTING: 进入新邮箱输入
    NEW_EMAIL_INPUTTING --> NEW_EMAIL_UNAVAILABLE: 新邮箱为空 / 格式错误 / 与当前邮箱相同 / 已被使用
    NEW_EMAIL_INPUTTING --> NEW_EMAIL_OTP_SENDING: 新邮箱可用，点击发送验证码
    NEW_EMAIL_OTP_SENDING --> NEW_EMAIL_OTP_SEND_FAILED: 新邮箱 OTP 发送失败
    NEW_EMAIL_OTP_SENDING --> NEW_EMAIL_OTP_SENT: 新邮箱 OTP 发送成功
    NEW_EMAIL_OTP_SENT --> NEW_EMAIL_OTP_FAILED: 新邮箱 OTP 失败 / 锁定 / 过期 / 重发超限
    NEW_EMAIL_OTP_SENT --> NEW_EMAIL_VERIFIED: 新邮箱 OTP 验证成功
    NEW_EMAIL_VERIFIED --> EMAIL_UPDATING: 提交账户邮箱更新
    EMAIL_UPDATING --> SUCCESS: 邮箱更新成功
    EMAIL_UPDATING --> UPDATE_FAILED: 更新失败 / 新邮箱被并发占用
    OLD_EMAIL_OTP_SEND_FAILED --> INIT: 用户返回 / 重试
    OLD_EMAIL_OTP_FAILED --> OLD_EMAIL_OTP_SENT: 按公共规则重试且未锁定
    OLD_EMAIL_VERIFIED --> INIT: 用户取消 / 返回个人中心
    NEW_EMAIL_UNAVAILABLE --> NEW_EMAIL_INPUTTING: 修改新邮箱
    NEW_EMAIL_OTP_SEND_FAILED --> NEW_EMAIL_INPUTTING: 用户修改邮箱 / 重试发送
    NEW_EMAIL_OTP_FAILED --> NEW_EMAIL_OTP_SENT: 按公共规则重试且未锁定
    UPDATE_FAILED --> NEW_EMAIL_INPUTTING: Try again
    SUCCESS --> [*]
```

| 状态 | 进入条件 | 允许操作 | 退出条件 | 失败 / 返回处理 | 数据结果 |
|---|---|---|---|---|---|
| INIT | 用户进入 Change Email Intro | 开始更换、返回个人中心 | 点击开始更换 | 返回个人中心时不保留流程状态 | Account 邮箱不变 |
| OLD_EMAIL_OTP_SENDING | 用户点击开始更换 | 等待发送结果 | 旧邮箱 OTP 发送成功 / 失败 | 发送失败按 Email OTP 公共规则处理，不进入 OTP 页 | Account 邮箱不变 |
| OLD_EMAIL_OTP_SENT | 旧邮箱 OTP 发送成功 | 输入 OTP、重发、返回 | OTP 验证成功 / 失败 / 锁定 / 过期 / 重发超限 | 失败、锁定、过期、重发超限按 Email OTP 公共规则处理 | Account 邮箱不变 |
| OLD_EMAIL_VERIFIED | 旧邮箱 OTP 验证成功 | 输入新邮箱、返回个人中心 | 用户提交新邮箱校验 | 返回个人中心视为取消本次更换流程 | Account 邮箱不变 |
| NEW_EMAIL_INPUTTING | 进入新邮箱输入页 | 输入 / 修改新邮箱、发送验证码、返回 | 新邮箱校验通过并请求发送 OTP | 校验失败停留当前页；返回个人中心视为取消本次更换流程 | Account 邮箱不变 |
| NEW_EMAIL_OTP_SENDING | 新邮箱校验通过后请求发送 OTP | 等待发送结果 | 新邮箱 OTP 发送成功 / 失败 | 发送失败停留 New Email Input，可重试发送或修改邮箱 | Account 邮箱不变 |
| NEW_EMAIL_OTP_SENT | 新邮箱 OTP 发送成功 | 输入 OTP、重发、返回 | OTP 验证成功 / 失败 / 锁定 / 过期 / 重发超限 | 失败、锁定、过期、重发超限按 Email OTP 公共规则处理；返回 New Email Input 后如修改邮箱需重新发送 OTP | Account 邮箱不变 |
| NEW_EMAIL_VERIFIED | 新邮箱 OTP 验证成功 | 等待 Account 更新邮箱 | 提交账户邮箱更新 | 不允许用户手动跳过更新；更新失败进入失败页 | Account 邮箱不变 |
| EMAIL_UPDATING | Account 正在更新邮箱 | 等待更新结果 | 更新成功 / 更新失败 / 新邮箱并发占用 | 更新失败进入 Change Email Failed；不触发成功通知 | 成功前 Account 邮箱不变 |
| SUCCESS | Account 邮箱更新成功 | 返回个人中心 | 用户点击返回 | 通知失败不回滚成功结果 | Account 邮箱已更新为新邮箱 |
| UPDATE_FAILED | Account 更新失败或新邮箱被并发占用 | 点击 Try again | 用户点击 Try again | 返回 New Email Input；不自动重试 | Account 邮箱保持原邮箱 |

**状态规则**

- 新邮箱 OTP 必须绑定当前账户、目标新邮箱和 `change_email` 场景；用户修改新邮箱后，旧的新邮箱 OTP 不得用于更新修改后的邮箱。
- 旧邮箱验证通过后仅代表允许进入新邮箱输入，不代表账户邮箱已变更。
- 新邮箱 OTP 验证成功后仍必须经过 Account 邮箱更新；只有 Account 更新成功才进入成功页。
- Account 更新成功前不得触发成功通知，不得刷新当前会话 email，不得展示成功页。
- Account 更新失败、并发占用或服务异常时，账户邮箱必须保持原邮箱，不产生半更新状态。
- 用户在当前前台流程内点击失败页 Try again 时，返回 New Email Input，不要求重新验证旧邮箱；如旧邮箱验证态已过期、用户退出后重新进入、更换流程被取消、或后端无法确认旧邮箱验证态有效，则必须重新从旧邮箱 OTP 开始。
- 用户从任一中间步骤返回个人中心，视为取消当前前台更换流程；已发送 OTP 是否仍可按公共规则验证，由 Email OTP Verification 控制，但再次修改目标邮箱必须重新发送新邮箱 OTP。
...
flowchart LR
  ...
  G[Change Email Failed]

  A -->|Click Email row| B
  B -->|Start change success| C
  C -->|Old email OTP verified| D
  D -->|New email available and OTP sent| E
  E -->|New email OTP verified and email updated| F
  E -->|Email update failed / new email occupied| G
  G -->|Try again| D
```
...
| 场景 / 元素 | 规则 | 不满足时提示 / 结果 | 后续流转 |
|---|---|---|---|
| 点击开始更换 | 请求发送旧邮箱 Email OTP | 发送失败时按 Email OTP 发送失败规则处理，账户邮箱不更新 | 发送成功进入 Old Email OTP Verify Page；发送失败停留当前页或按公共规则提示 |
...
| 后端校验通过 | 只发送新邮箱 OTP，不更新账户邮箱 | 新邮箱 OTP 发送失败时按 Email OTP 发送失败规则处理 | 发送成功进入 New Email OTP Verify Page；发送失败停留当前页 |
...
| 场景 | 后续流转 |
|---|---|
| 新邮箱 OTP 验证成功，且 Account 更新邮箱成功 | 进入 Change Email Success |
| 新邮箱 OTP 验证成功，但 Account 更新邮箱失败 | 进入 Change Email Failed；账户邮箱保持原邮箱；不触发成功通知 |
| 新邮箱 OTP 验证成功，但更新前发现新邮箱已被并发占用 | 进入 Change Email Failed；账户邮箱保持原邮箱；提示 `This email has been used` |
...
### 3.8 失败页：Change Email Failed

![Change Email Failed](./assets/change-email/change-email-failed.svg)

> 本页用于承接“新邮箱 OTP 已验证成功，但最终账户邮箱未更新成功”的失败结果。OTP 输入错误、锁定、过期、重发超限等公共 OTP 失败不进入本页，仍按 Email OTP Verification 公共页面规则处理。

**页面目的**  
告知用户邮箱更换未完成，账户邮箱仍保持原邮箱，并提供重新尝试路径。

**展示规则**
- 页面标题：`Email change failed`
- 当失败原因为新邮箱已被并发占用时，提示：`This email has been used`
- 当失败原因为 Account 更新失败、服务异常或未知错误时，提示：`Something went wrong. Please try again later`
- 页面必须明确表达：邮箱未更换成功，当前账户邮箱仍为原邮箱。
- 如展示邮箱，原邮箱和目标新邮箱均按 Email OTP Verification 掩码规则展示。
- 页面只保留一个主按钮：`Try again`。

**交互与失败后处理**

| 场景 / 处理项 | 规则 | 结果 |
|---|---|---|
| 点击 Try again | 返回 New Email Input | 当前旧邮箱验证态仍有效时，不要求重新验证旧邮箱；用户需重新确认新邮箱，并重新发送 / 验证新邮箱 OTP |
| 旧邮箱验证态失效后点击 Try again | 返回 Old Email OTP Verify Page | 用户需重新验证旧邮箱后再继续更换邮箱 |
| 点击导航返回 | 返回上一页或个人中心，按 App 通用导航规则处理 | 账户邮箱保持原邮箱 |
| 数据变更 | Account 邮箱不得更新 | 后续登录账号、找回密码邮箱、Email OTP 接收邮箱仍使用原邮箱 |
| 会话 / 缓存刷新 | 不刷新当前会话 email 为新邮箱 | 当前会话 email 保持原邮箱 |
| 通知触发 | 不触发邮箱更换成功通知 | 旧邮箱 / 新邮箱均不发送成功通知 |
| 操作日志 | 记录邮箱更换失败操作日志，用于问题排查和风控追踪 | 不影响用户前台流程 |
...
- Prototype: `requirements/2026-05/account/assets/change-email/change-email-failed.svg`
