...
sequenceDiagram
    ...
    User->>App: 从个人中心进入 Change Email
    App-->>User: 展示更换邮箱入口页

    User->>App: 开始更换并验证旧邮箱
    App->>Security: 复用 Email OTP 验证旧邮箱
    Security-->>App: 返回旧邮箱验证结果

    alt 旧邮箱验证未通过
        App-->>User: 按 Email OTP 公共规则处理，邮箱不变更
    else 旧邮箱验证通过
        User->>App: 输入新邮箱
        App->>Account: 校验新邮箱是否可用
        Account-->>App: 返回新邮箱校验结果

        alt 新邮箱不可用
            App-->>User: 停留新邮箱输入页，展示错误
        else 新邮箱可用
            App->>Security: 复用 Email OTP 验证新邮箱
...
| UPDATE_FAILED | Account 更新失败或新邮箱被并发占用 | 点击 Try again | 用户点击 Try again | 返回 Change Email Intro，重新开始更换流程 | Account 邮箱保持原邮箱 |
...
- 用户在失败页点击 Try again 时，统一返回 Change Email Intro，重新开始更换流程；不保留旧邮箱验证态，不自动跳过旧邮箱 OTP。
...
| 场景 / 处理项 | 规则 | 结果 |
|---|---|---|
| 点击 Try again | 返回 Change Email Intro | 用户重新开始更换邮箱流程，需要重新验证旧邮箱 |
