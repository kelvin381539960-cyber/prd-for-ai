...
KYC 由 AIX 客户端、AIX 服务端、DTC、AAI 协作完成。AIX 负责用户流程与状态编排，DTC / AAI 提供外部开户、身份识别和审核结果。

| 系统 | 主要职责 |
|---|---|
| AIX 客户端 | 展示用户侧 KYC 页面，承接国家选择、协议确认、证件识别入口、人脸验证入口、POA 上传和结果页。 |
| AIX 服务端 | 编排 KYC 流程，处理状态判断、国家准入、waitlist、外部接口调用、状态保存和错误码转换。 |
| DTC | 提供外部开户和 KYC 结果能力，包括 verification URL、`clientStatus`、POA 审核结果和 Sub Account 相关能力。 |
| AAI | 提供身份识别能力，包括 Passport OCR、人脸识别 / 人脸比对、POA OCR 等结果。 |
...
| 顺序 | 模块 | 说明 |
|---|---|---|
| 1 | 入口与状态分流 | 用户进入 KYC 后，系统判断是继续认证、展示结果、进入 waitlist，还是阻止继续。 |
| 2 | 居住国家与准入判断 | 用户选择或确认居住国家；系统判断该国家是否支持开户。 |
| 3 | 协议确认 | 用户确认开户协议和 Reverse Solicitation Declaration；系统保存确认结果。 |
| 4 | 证件识别 | 用户完成 Passport OCR；系统根据识别结果决定继续或返回失败原因。 |
| 5 | 人脸验证 | 用户完成活体采集和人脸比对；系统处理成功、失败、重试、锁定和超时。 |
| 6 | 地址证明 | 用户上传 POA；系统校验文件、识别地址信息，并判断居住国家是否匹配。 |
| 7 | 提交审核与结果处理 | 用户提交 KYC 后，系统同步外部审核结果，并维护 AIX / DTC 状态。 |
| 8 | 失败与异常处理 | 统一处理认证失败、网络异常、服务异常、超时、锁定和错误码文案。 |
| 9 | 结果通知边界 | 仅记录通知规则的待确认边界；具体模板、渠道和补发策略不在本文展开。 |
