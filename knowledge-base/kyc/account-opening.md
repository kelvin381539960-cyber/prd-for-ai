...
| 场景 | 处理 | 细则 |
|---|---|---|
| 无法获取协议 | Toast：`Something went wrong. Please try again later`，不允许继续 | 影响协议区和底部认证按钮 |
| Reverse Solicitation 缺失 | DTC 返回 `50013`；阻止生成验证 URL，需补声明后再继续 | 见 [5.2 get-verification-url 错误码](#52-get-verification-url-错误码) |
