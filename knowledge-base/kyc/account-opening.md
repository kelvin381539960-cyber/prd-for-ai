...
| 开户链路 | 模块 | 主要负责内容 |
|---|---|---|
| 1 | KYC 入口与状态分流 | 用户进入开户认证后，根据当前 KYC 状态、waitlist 状态和异常情况决定继续认证、展示结果或阻止继续。 |
| 2 | 居住国家与准入判断 | 用户选择或确认居住国家后，判断该国家是否支持开户；不支持或受限时进入 waitlist。 |
| 3 | 协议确认 | 用户确认开户相关协议和 Reverse Solicitation Declaration，系统保存确认结果并用于后续开户入参。 |
| 4 | 证件识别 | 用户进入 Identity Verify，通过 Passport OCR 完成证件识别，并根据识别结果继续或返回失败原因。 |
| 5 | 人脸验证 | 用户完成人脸采集和人脸比对；系统处理成功、失败、重试、锁定和超时等结果。 |
| 6 | 地址证明 | 用户上传 POA 地址证明；系统校验文件、识别地址信息，并判断居住国家是否匹配。 |
| 7 | 提交审核与结果处理 | 用户提交 KYC 后，系统同步外部审核结果，并维护 AIX 页面状态、DTC `clientStatus`、文件审核状态和最终 KYC 结果。 |
| 8 | 失败与异常处理 | 覆盖证件失败、人脸失败、POA 失败、网络异常、服务异常、超时、锁定和错误码文案映射。 |
| 9 | 结果通知边界 | 仅记录 KYC 结果通知的待确认边界；具体通知模板、渠道和补发策略不在本文展开。 |
