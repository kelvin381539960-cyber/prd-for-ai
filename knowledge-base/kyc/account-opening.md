...
如果 AIX 钱包要承接开户、法币出入金、卡、支付网络或托管账户等金融能力，就需要先确认用户身份、居住地和地址证明是否符合准入要求；否则无法判断用户是否可以被外部金融合作方接受，也无法控制地区限制、身份不明和资料缺失带来的合规风险。

因此需要建设 KYC 功能，把国家准入、协议确认、证件识别、人脸验证、地址证明和审核状态串成一条可执行的开户准入流程。
...
| AIX 服务端 | 负责 KYC 流程编排和状态判断，包括查询用户 KYC 状态、判断国家是否支持、判断 waitlist、调用外部认证能力、保存关键状态、转换错误码并返回给客户端。 |
...
| 模块 | 主要负责内容 |
|---|---|
| 国家准入与 Waitlist | 判断用户居住国家是否支持开户；不支持或受限时进入 waitlist，当前流程不允许继续 KYC。 |
| 协议确认 | 处理开户相关协议和 Reverse Solicitation Declaration 的确认、保存和传参。 |
| 证件识别 | 通过 Passport OCR 完成证件识别，并根据识别结果继续流程或返回失败原因。 |
| 人脸验证 | 处理活体采集、人脸比对、失败重试、锁定和超时等场景。 |
| 地址证明 | 处理 POA 文件上传、地址证明识别、居住国家匹配和审核结果。 |
| KYC 状态与结果 | 维护 AIX 页面状态、DTC `clientStatus`、文件审核状态和最终 KYC 结果之间的对应关系。 |
| 异常与错误码 | 统一处理网络异常、服务异常、外部认证失败、超时、锁定和错误码文案映射。 |
| 通知规则 | 仅记录 KYC 结果通知的待确认边界；具体通知模板、渠道和补发策略不在本文展开。 |
