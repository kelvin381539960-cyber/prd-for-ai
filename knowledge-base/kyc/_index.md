...
version: "2.1"
status: active
source_doc: knowledge-base/kyc/account-opening.md；knowledge-base/common/aai.md；knowledge-base/changelog/knowledge-gaps.md；knowledge-base/_system-boundary.md；用户确认结论 2026-05-02
source_section: KYC 使用态目录结构；Account Opening / KYC；AAI Dependency；ALL-GAP 总表；system-boundary
...
| `kyc/wallet-kyc.md` | deprecated | 旧 Wallet KYC 占位文件；不再作为运行时主事实源 |
...
| `kyc/wallet-kyc.md` | 命名容易误导，后续应删除或归档；当前引用已切换到 `kyc/account-opening.md` |
...
| ALL-GAP-032 | Wallet 开户触发时机 | Account Opening PRD 已可回填部分事实，剩余入口覆盖范围待确认 |
| ALL-GAP-033 | KYC 失败 / 重试 / 人工处理规则 | 页面失败、Face 锁定可回填；人工处理与外部错误码仍待确认 |
...
6. 不默认读取 `kyc/wallet-kyc.md`、moved notice、stage-review 或历史 checklist 文件。
