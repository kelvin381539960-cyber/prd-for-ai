...
version: "1.0"
status: active
source_doc: knowledge-base/wallet/_index.md；knowledge-base/wallet/balance.md；knowledge-base/wallet/deposit.md；knowledge-base/wallet/receive.md；knowledge-base/transaction/history.md；knowledge-base/transaction/reconciliation.md；knowledge-base/kyc/account-opening.md；knowledge-base/changelog/knowledge-gaps.md；knowledge-base/changelog/decision-log/2026-05-02-deposit-active.md；knowledge-base/changelog/decision-log/2026-05-02-receive-deferred.md
...
| WalletConnect / Self-custodial Wallet 充值 | `wallet/deposit.md` | `integrations/walletconnect/_index.md`、`transaction/history.md` | WC 与 ActivityType、relatedId、payment_info 映射见 ALL-GAP-002 / 007 / 012 / 014 |
...
| 3 | 公共能力文件 | `integrations/walletconnect/_index.md`、`common/notification.md`、`common/errors.md`、`integrations/dtc/_index.md` |
| 4 | ALL-GAP 总表 | 仅用于引用未确认项，不得当成已确认事实 |
| 5 | decision-log | 用于解释状态、边界和迁移决策 |
| 6 | deferred / placeholder 文件 | `receive.md` 等，仅用于说明不可用或待确认范围 |
...
2. 不得把 Deposit 的 GTR 路径和 WalletConnect 路径默认合并。
3. 不得把 DTC Available Currency 直接等同于 AIX 前端展示币种。
4. 不得把 DTC numeric state 直接等同 Wallet `state` 或用户展示文案。
5. 不得把 `FIAT_DEPOSIT=6` 必然等同 GTR。
6. 不得把 `CRYPTO_DEPOSIT=10` 必然等同 WalletConnect。
7. 不得把 Deposit success 必然等同 Wallet `COMPLETED` 或余额立即可用。
8. 不得把 Risk Withheld 直接等同 Wallet `REJECTED / PENDING / PROCESSING`。
9. 不得在 Wallet 目录补写 KYC、Transaction History、Notification、Reconciliation 的主事实。
...
1. `wallet/_index.md`
2. `wallet/deposit.md`
3. 如涉及 WalletConnect 细节，再读 `integrations/walletconnect/_index.md`
