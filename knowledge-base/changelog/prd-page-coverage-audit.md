...
| Out of scope / 暂无 runtime KB 承接文件，已转 non-runtime visual index | 245 |
| 删除线来源，不绑定 runtime | 3 |
| 仍需复核 | 0 |
...
4. Website / OBoss / MGM / Popup Banner / Card Me 等当前没有 runtime KB 承接文件的页面，已单独承接到 `knowledge-base/non-runtime/`，状态为 visual-only，不混入已有业务事实文件。
