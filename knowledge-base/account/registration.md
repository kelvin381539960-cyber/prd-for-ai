...
sequenceDiagram
    ...
    Note over 用户,Security: A. 发起注册
    用户->>App: 进入注册
    App-->>用户: 展示注册页面

    用户->>App: 提交注册信息
    App->>Account: 发起注册前校验
    Account->>Account: 校验邮箱唯一性、推荐码、频控与协议可用性

    alt 注册前校验不通过
        Account-->>App: 返回不通过原因
        App-->>用户: 注册停留在当前阶段，按页面规则提示
    else 注册前校验通过
        Account->>Security: 发起邮箱 OTP 验证
        Security-->>用户: 进入邮箱 OTP 验证
    end

    Note over 用户,Security: B. 邮箱验证
    用户->>Security: 完成邮箱 OTP 验证
    ...
    Note over 用户,Account: C. 设置登录密码
    用户->>App: 提交登录密码
    App->>App: 校验密码规则

    alt 密码规则不通过
        App-->>用户: 停留密码设置阶段，按页面规则提示
    else 密码规则通过
        App-->>用户: 进入确认密码
    end
    ...
    alt 确认密码不通过
        App-->>用户: 停留确认密码阶段，按页面规则提示
    else 确认密码通过
        App->>Account: 创建账户并设置登录密码
        Account->>Account: 生成 UID、设置账户 Active、绑定当前 Device ID
        Account-->>App: 注册成功并完成自动登录
        App-->>用户: 进入 AIX Tag 设置
    end

    Note over 用户,Account: D. 设置或跳过 AIX Tag
    用户->>App: 提交 AIX Tag
    ...
    alt 用户跳过 AIX Tag
        ...
        alt Tag 基础规则不通过
            App-->>用户: 停留 AIX Tag 设置阶段，按页面规则提示
        else Tag 基础规则通过
            App->>Account: 校验并创建 AIX Tag
