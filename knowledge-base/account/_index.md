...
Account 模块沉淀 AIX 用户账户体系事实，覆盖注册、登录、基础账户状态、唯一性和设备绑定规则。Password Reset 因历史 PRD 正文为删除线，当前仅保留为 NEED_CONFIRMATION。
...
| Login | [login.md](./login.md) | need_confirmation | Login Page、Quick Login、Enable BIO；输入方式存在删除线内容，需要确认当前 Email / Phone 范围 | registration-login / 7.2；security / 认证能力 |
| Password Reset | [password-reset.md](./password-reset.md) | need_confirmation | 历史 PRD 7.3 整体为删除线，不能作为 active runtime fact | registration-login / 7.3；security / 认证能力矩阵 |
...
| BIO 清理 | 历史 PRD 有相关痕迹但正文为删除线 | 需确认当前是否生效 | registration-login / 7.3.1，NEED_CONFIRMATION |
