...
Account 模块沉淀 AIX 用户账户体系事实，覆盖注册、登录、基础账户状态、唯一性和设备绑定规则。Password Reset 因历史 PRD 正文为删除线，当前仅保留为 ALIGNED。
...
| Login | [login.md](./login.md) | active | Login Page、Quick Login、Enable BIO；输入方式存在删除线内容，需要确认当前 Email / Phone 范围 | registration-login / 7.2；security / 认证能力 |
| Password Reset | [password-reset.md](./password-reset.md) | active | 历史 PRD 7.3 整体为删除线，不能作为 active runtime fact | registration-login / 7.3；security / 认证能力矩阵 |
...
| 账户状态 | Active / Banned / Closed | registration-login / 5.2 账户说明 | `Locked` 在 converted-prd 中为删除线，不沉淀为账户状态；认证锁定属于 Security 场景锁定 |
...
| Locked | 不作为账户状态沉淀 | 源 PRD 中为删除线 | 不适用 | 若出现 OTP / Email OTP / Face Auth 锁定，应归入 Security 认证锁定，不归入 Account Status | 否 |
...
| 规则 / 字段 | 规则内容 | 影响范围 | 来源 |
|---|---|---|---|
| UID | 服务端在用户注册成功后生成；生成规则由开发侧定义，但产品诉求是便于 CS、MGM 等人工记录与核对场景识别、记忆和沟通 | 全局账户识别 / 客服 / MGM | AIX Card 注册登录需求V1.0 / 5.2.1 |
...
| 账户不可登录 | Banned / Closed | 阻止登录 | login.md / status-dictionary.md |
| 密码重置成功 | 历史 PRD 7.3 为删除线 | 不作为当前已确认事实 | password-reset.md |
...
| 账户状态拦截 | Banned / Closed 影响登录能力；Locked 不作为账户状态沉淀 | 登录与后续业务入口 | registration-login / 5.2.2 |
| 身份认证 | 注册、登录、密码重置依赖 Security 模块 | 账户安全 | AIX Security 身份认证需求V1.0 |
| BIO 清理 | 历史 PRD 有相关痕迹但正文为删除线 | 需确认当前是否生效 | registration-login / 7.3.1，ALIGNED |
...
- 2026-05-09：用户确认删除线内容等同已删除。Account 模块不再把 Password Reset / Forgot Password 删除线逻辑标为待确认，而是从 runtime 逻辑中排除。
- Login / Registration / Security 的 active 事实继续保留；Forgot Password / Reset Password 不作为当前 App runtime 能力。
