...
| 国家 / 地区选择 | 手机号登录展示全球国家 / 地区区号列表 | AIX Card 注册登录需求V1.0 / 7.2.4.1 | 中国和中国台湾选项隐藏 |
...
| 本地存在 BIO 密钥 | 仅检测到可用 Biometric 密钥时展示 Quick Login | AIX Card 注册登录需求V1.0 / 7.2.5 |
| 设备绑定 | 仅当设备已绑定，登录时方可启用 Biometric | AIX Card 注册登录需求V1.0 / 6.2.6 |
...
| Enable now 5 分钟内 | 手动登录完成 5 分钟内点击 Enable now | 调起设备生物识别 | 免再次身份认证 | 设置成功后进入 Home | 设备验证失败按 Biometric 处理 |
| Enable now 超过 5 分钟 | 手动登录完成超过 5 分钟点击 Enable now | 进入身份验证流程 | Security 认证通过后继续设备生物识别 | 设置成功后进入 Home | 认证失败 / 设备失败 |
...
| 元素 | 类型 | 展示条件 | 交互规则 | 异常 |
|---|---|---|---|---|
| Email / Phone 切换 | Tab | 默认展示 | 默认选中 Email；用户可切换到 Phone | 切换时是否保留输入内容详见 `knowledge-gaps.md` |
...
| Phone 少于 6 位 | Phone 输入不满足最小长度 | `Phone number must be at least 6 digits` | 当前知识库旧内容；详见 `knowledge-gaps.md` |
...
| 元素 | 类型 | 展示条件 | 交互规则 | 异常 |
|---|---|---|---|---|
| 国家 / 地区列表 | List | Phone tab 点击 Country Code | 展示全部国家 / 地区；按排序规则展示 | 中国和中国台湾选项隐藏 |
...
| Android Fingerprint | Device Auth | Android 设备且支持指纹 | 设备验证通过后进行后端验证，并使用 biometric 签名请求身份认证 | 后端失败弹窗提示；原文疑似串文详见 `knowledge-gaps.md` |
...
| Image / Title / Subtitle | Content | 页面展示时 | 固定文案 | 无 |
...
| Phone 少于 6 位 | Phone 长度不足 | `Phone number must be at least 6 digits` | 阻止 Next | 留在 Login Page | 当前知识库旧内容；详见 `knowledge-gaps.md` |
...
- (Ref: knowledge-base/changelog/knowledge-gaps.md / Account Login / 2026-05-01)
