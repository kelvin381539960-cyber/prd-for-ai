...
Card 模块沉淀 AIX Card 的申请、卡首页、实体卡激活、PIN、卡敏感信息查看、锁卡 / 解锁、卡状态、卡字段和卡交易资金回退规则。
...
| Card Status & Fields | [card-status-and-fields.md](./card-status-and-fields.md) | active | 卡状态、卡类型、品牌、费用类型、自动扣款字段、接口路径和操作限制缺口 | AIX Card V1.0【Application】 / 3-4；AIX Card manage模块需求V1.0 / 6.1-6.4 |
...
| Card Activation | [activation.md](./activation.md) | active | 实体卡激活、卡号后四位校验、Face Authentication、激活接口、Set PIN 入口 | AIX Card manage模块需求V1.0 / 7.2 |
| PIN | [pin.md](./pin.md) | active | Set PIN / Change PIN / Reset PIN、PIN 公钥、OTP For Reset PIN、Reset Card PIN | AIX Card manage模块需求V1.0 / 7.3；名词解释 PIN |
| Sensitive Info | [sensitive-info.md](./sensitive-info.md) | active | 卡敏感信息查看流程、认证边界、展示规则、接口依赖与失败处理 | AIX Card manage模块需求V1.0 / 7.1；AIX Card V1.0【Application】 / 2.2 |
...
| 卡状态允许操作 | 激活、PIN、Lock、Unlock、卡敏感信息查看、交易能力均受卡状态限制 | AIX Card manage模块需求V1.0 / 6.4 |
...
| 卡管理 | 用户触发激活、PIN、Lock、Unlock、卡敏感信息查看 | 按卡状态与 Security 认证规则处理 | 完成对应卡操作 | 失败按功能文件处理 |
...
| `Get Card Sensitive Info` | 查询卡敏感信息 | AIX Card V1.0【Application】 / 2.2 / 6.4 | 需完成 Face Authentication |
...
| Face Authentication 前置 | Card Application、卡敏感信息查看、激活卡、设置 PIN、重置 PIN 等使用 Face Authentication | 复用 Security 高强度认证 | AIX Security 身份认证需求V1.0 / 7.2；AIX Card V1.0【Application】 / 2.2 |
