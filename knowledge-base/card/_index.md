...
| 功能 | 文件 | 状态 | 说明 | 来源 |
|---|---|---|---|---|
| Card Application | [application.md](./application.md) | 未开始 | 新用户申请虚拟卡 / 实体卡，包含申卡资格、卡类型、费用、币种、地区、自动扣款 | AIX Card V1.0【Application】 / 2.1 / 5.1 |
| Card Home | [card-home.md](./card-home.md) | 未开始 | 已申卡用户查看卡片、卡面、卡类型、余额、操作入口、FAQ、交易记录入口 | AIX Card V1.0【Application】 / 5.2；AIX APP V1.0【Home】 |
| Card Activation | [activation.md](./activation.md) | 未开始 | 实体卡激活流程 | AIX Card manage模块需求V1.0 / 7.2 |
...
| Card Status & Fields | [card-status-and-fields.md](./card-status-and-fields.md) | 未开始 | 卡状态、卡类型、品牌、费用类型、自动扣款字段和操作限制 | AIX Card V1.0【Application】 / 3-4；AIX Card manage模块需求V1.0 / 6.1-6.4 |
...
```text
Wallet Opened + KYC Passed → Face Authentication → Card Application → Card Home → Card Manage / Card Transaction
```
...
flowchart LR
    ...
    Application -->|Success / pending| CardHome
    Application -.->|Failed| Error

    Entry -->|Has card / pending card| CardHome
...
| 字段 / 接口 / 能力 | 用途 | 来源 | 备注 |
|---|---|---|---|
| `Card Application` | 申请虚拟卡或实体卡 | AIX Card V1.0【Application】 / 2.2 / 6.1 | `[POST] /openapi/v1/card/requestcard` |
