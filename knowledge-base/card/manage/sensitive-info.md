...
| 状态 | Review |
| 更新时间 | 2026-05-05 |
| 来源文档 | AIX Card Manage、DTC Card Issuing API、Security Face Authentication、Standard PRD Template v1.3 |
...
用户需要在安全认证后查看完整卡片信息，例如 PAN、EXP、CVV / CVC 和 Name on card。由于这些字段属于敏感信息，必须明确查看权限、认证要求、字段来源、复制规则和失败处理。

### 2.2 用户问题 / 业务问题

如果敏感信息查看与卡状态、认证规则和接口失败处理未统一，可能造成 PAN / CVC 泄露，或接口失败时展示不完整敏感数据。
...
| 防截屏 / 超时隐藏 | Deferred | P1 | Open | 当前事实未定义 |
...
| 认证失败 | Face Authentication 失败 | Security | 认证失败 | 按 Security 规则 | 不查询敏感信息 | 视规则 | 原状态 |
...
flowchart LR
    ...
    FaceAuth -->|Verified| Popup
    FaceAuth -->|Failed| Error
    Popup -->|Close| CardHome
...
|---|---|---|---|---|---|---|
| 接口 | Get Card Basic Info | DTC | DTC API | 查询基础卡信息 | 优先 `[POST] /openapi/v1/card/inquiry-card-info` | 失败 Toast |
| 接口 | Get Card Sensitive Info | DTC | DTC API | 查询敏感卡信息 | 优先 `[POST] /openapi/v1/card/inquiry-card-sensitive-info` | 失败 Toast，不展示敏感信息 |
